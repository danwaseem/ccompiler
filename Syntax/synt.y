...
SDEFINE	: STRUCTUNION IDEN { insert(); } OB V1  CB SEMICOLON;

V1 	: VDECLARE V1
		| ;

SDECLARE	: STRUCTUNION IDEN VDECLAREL;
...
FDECLARETYPE	: TYPE IDEN OP  { insert();};
...
PARAMIDEN	: IDEN { insert(); } PARAMIDENBR;

PARAMIDENBR		: OSB CSB
							| ;

COMPST	: OB STATEMENTLIST CB ;
...
SEXPB	: OR ANDEXPR SEXPB | ;

ANDEXPR	: UNARY ANDEXPRBR;

ANDEXPRBR	: AND UNARY ANDEXPRBR
					| ;

UNARY	: EXC UNARY
			| REXPR ;

REXPR	: SUMEXPR REXPRBR;

REXPRBR	: RELATIONAL SUMEXPR
				| ;
...
SUMEXPR	: SUMEXPR sum_operators TERM
				| TERM ;
...
MUTABLEBR	: OSB EXPR CSB
					| FULLSTOP IDEN;

IMMUTABLE	: OP EXPR CP
					| CALL
					| constant;

CALL	: IDEN OP ARGUMENTS CP;
...
constant	: INTEGER 	{ insertvariable(); }
